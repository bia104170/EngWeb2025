...
router.get('/', function(req, res) {
  ...
  axios.get('http://localhost:3000/alunos')
    .then(resp=> {
      res.status(200).render('studentsListPage', {lalunos: resp.data,  'date': date, title: 'Página Inicial'})
    })
    .catch(error => {
      console.log(error)
...
router.get('/delete/:id', function(req, res) {
  ...
  axios.get('http://localhost:3000/alunos')
    .then(resp => {
      ...
      if (aluno) {
        res.render('studentDeletePage', {aluno: aluno, date: date, title: 'Remover aluno'});
      } else {
